...
def get_inputs():
    ...
    # inputs = ["C:/Users/mihai/OneDrive/Desktop/test/TestStreams/funky_sample.wav", "C:/Users/mihai/OneDrive/Desktop/test/TestStreams/WhiteNoise.wav"]
    inputs = [os.path.join(input_streams_root, x) for x in ['funky_sample.wav', 'WhiteNoise.wav', 'Freq_sweep.wav']]
...
def get_params():
    ...
    params = [
        ["On", "Off"],					# enable
        ["-0.999", "-4", "-1513", "-0.01"],	    # Gain
        ["3_2_1", "2_2_0", "2_0_0"],	# Mode
        # [0, 1],  				# mute
        # ["0.999", "-0.999"],	# gain 
        # [0, 1, 2]				# output mode
    ]
...
def run_test(case):
    ...
    if models.__len__() > 1:
        with open(os.path.join(test_dir, 'compare_log.txt'), 'w') as log_file:
